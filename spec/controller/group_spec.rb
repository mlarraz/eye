...
RSpec.describe "Eye::Group" do
  ...
  describe "Chain calls" do
    ...
    it "should call chain_schedule for start" do
      ...
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, 7, { command: :start })
      @g.start
    end

    it "should call chain_schedule for start, with type sync" do
      @g = Eye::Group.new('gr', {:chain => {:start => {:type => :sync, :command => :start, :grace => 7}}})
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:sync, 7, { command: :start })
      @g.start
    end
    ...
    it "config for start and restart, use both" do
      ...
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:sync, 8, { command: :restart })
      @g.restart
    end

    it "should use options type" do
      @g = Eye::Group.new('gr', {:chain => {:start => {:type => :sync, :command => :start}}})
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:sync, Eye::Group::DEFAULT_CHAIN, { command: :start })
      @g.start
    end

    it "with empty grace, should call default grace 0" do
      @g = Eye::Group.new('gr', {:chain => {:start => {:command => :start}}})
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, Eye::Group::DEFAULT_CHAIN, { command: :start })
      @g.start
    end
    ...
    it "chain options for restart, but called start, should call chain but with default options" do
      ...
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, Eye::Group::DEFAULT_CHAIN, { command: :restart })
      @g.restart
    end
    ...
    it "when chain clearing by force" do
      ...
      expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, 0, { command: :monitor })
      @g.monitor

      expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, 0, { command: :restart })
      @g.restart

      expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, 0, { command: :start })
      @g.start
    end

    it "with params" do
      @g = Eye::Group.new('gr', {})
      expect(@g.wrapped_object).to receive(:fast_call).with({ :command => :signal, :args => [15] })
      @g.signal(15)
    end
    ...
    describe "monitor using chain as start" do
      it "monitor call chain" do
        ...
        expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, 3, { command: :monitor })
        @g.monitor
      end

      it "monitor not call chain" do
        @g = Eye::Group.new('gr', {:chain => {:restart => {:command => :restart, :grace => 3}}})
        expect(@g.wrapped_object).to receive(:chain_schedule).with(:async, Eye::Group::DEFAULT_CHAIN, { command: :monitor })
        @g.monitor
      end
    end
  end

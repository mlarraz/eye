...
describe "Eye::Group" do
  ...
  describe "Chain calls" do
    ...
    it "config for start and restart, use both" do
      ...
      mock(@g).chain_schedule(:sync, 8.0, {command: :restart})
      @g.restart
    end
    ...
    it "when chain clearing by force" do
      ...
      mock(@g).chain_schedule(:async, 0.0, {command: :monitor})
      @g.monitor

      mock(@g).chain_schedule(:async, 0.0, {command: :restart})
      @g.restart

      mock(@g).chain_schedule(:async, 0.0, {command: :start})
      @g.start
    end
    ...
    describe "monitor using chain as start" do
      it "monitor call chain" do
        ...
        mock(@g).chain_schedule(:async, 3.0, {command: :monitor})
        @g.monitor
      end
